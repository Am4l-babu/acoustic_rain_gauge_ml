...
nobody watching it:
  ...
  - Every experiment's result is written to docs/reports/sweep_progress.json
...
in place):
    ...
    python3 src/training/run_sweep.py
...
in docs/reports/sweep_progress.json.
"""
import argparse
import json
import subprocess
...
def run_dl_phase(progress, args):
    ...
    for arch, lr, batch, tag in experiments:
        ...
        cmd = [
            PYTHON, str(REPO_ROOT / "src" / "training" / "train_dl_model.py"),
            "--full", "--models", arch,
