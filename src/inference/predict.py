...
Run:
    python src/inference/predict.py path/to/clip.wav
    python src/inference/predict.py path/to/clip.wav --threshold 0.5
    python src/inference/predict.py path/to/clip.wav --no-mm
"""
...
from features.data_cleaning import _extract_features, _dur_category
from dl.dl_dataset import CLIP_SAMPLES, MFCCExtractor
from dl.dl_models import MODEL_REGISTRY
from features.master_feature_extraction import extract_all_features
...
def predict_ensemble(wav_path: str) -> dict:
    ...
    and combination logic that turned out to be, and docs/reports/ensemble_stack_report.json
...
def main():
    ...
    parser.add_argument("--ensemble", action="store_true",
                         help="Use the stacked CNN + XGBoost-optimized ensemble instead of the "
                              "Stage 4/5 baseline pipeline (higher accuracy, see docs/reports/ensemble_stack_report.json)")
